 ...
 * @author sdedic
 */
public class ConvertingBufferedModel extends AbstractValueModel implements BufferedModel, Lookup.Provider {
    ...
    private final Supplier<Lookup> lkp;
    ...
    public ConvertingBufferedModel(ValueModel source, BindingConverter converter) {
        ...
        if (source instanceof Lookup.Provider) {
            if (converter instanceof Lookup.Provider) {
                Lookup l =  new ProxyLookup() {
                    @Override
                    protected void beforeLookup(Lookup.Template<?> template) {
                        setLookups(((Lookup.Provider)source).getLookup(), ((Lookup.Provider)converter).getLookup());
                        super.beforeLookup(template);
                    }
                };
                lkp = () -> l;
            } else {
                lkp = ((Lookup.Provider)source)::getLookup;
            }
        } else if (converter instanceof Lookup.Provider) {
            lkp = ((Lookup.Provider)converter)::getLookup;
        } else {
            lkp = new Supplier<Lookup>() {
                Lookup inst;
                
                @Override
                public synchronized Lookup get() {
                    if (inst == null) {
                        inst = createLookup();
                    }
                    return inst;
